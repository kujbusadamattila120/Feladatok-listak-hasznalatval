...
namespace ujSzamsork
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int atlag = osszeg / szamsorA.Count();
            Console.WriteLine($"Átlag: {atlag}");
            //4
            List<int> ujSzamsor = new List<int> { };
            foreach (int sz in szamsorA)
            {
                ujSzamsor.Add(sz);
            }

            foreach (int sz in szamsorB)
            {
                ujSzamsor.Add(sz);

            }
            for (int i = 0; i < ujSzamsor.Count - 1; i++)
            {
                for (int j = 0; j < ujSzamsor.Count - i - 1; j++)
                {
                    if (ujSzamsor[j] > ujSzamsor[j + 1])
                    {
                        int temp = ujSzamsor[j];
                        ujSzamsor[j] = ujSzamsor[j + 1];
                        ujSzamsor[j + 1] = temp;
                       
                    }
                }
            }
            foreach (int szam in ujSzamsor)
            { Console.WriteLine(szam); }
            //5
            int canonCounter = 0;
            foreach (string g in tintapatronok)
            {
                if (g.Contains("Canon"))
                {
                    canonCounter++;
            ...
            Console.WriteLine("Canon patronok száma: " + canonCounter);
